namespace Example.Arithmetic.VectorProduct
{
    ...
    public class ProductBehaviour : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!productDirectly)
                Debug.LogFormat("CROSS: {0}×{1} = {2}, Size : {3}", transform.forward, direction, Vector3.Cross(transform.forward, direction), Vector3.Cross(transform.forward, direction).magnitude);
            else
                Debug.LogFormat("CROSS: {0}×{1} = {2}, Size : {3}", transform.forward, direction, Cross(transform.forward, direction), Cross(transform.forward, direction).magnitude);
        }
    }
}
